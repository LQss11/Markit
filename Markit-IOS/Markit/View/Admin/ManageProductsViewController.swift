            ...
            if let arr = data.arrayObject as? [[String:AnyObject]] {
                for items in arr{
                    ...
                    if let savedPerson = userDef.object(forKey: "user") as? Data {
                        ...
                        if( items["company"]!.stringValue == loadedPerson!.company)
                        {
...
extension ManageProductsViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if (tableView.cellForRow(at: indexPath) as? ProductCell) != nil {
            ...
            let vc = self.storyboard?.instantiateViewController(withIdentifier: "ViewProductStoryboard") as! ViewProductViewController
              self.present(vc, animated: true,completion: nil)
